...
library(data.table)
  ...
  acsdems<-read.delim(paste0(datapath, 'acspsw03.txt'), skip=2, header=F, sep = "\t")
  cols_acsdems<-read.delim(paste0(datapath, 'acspsw03.txt'), header=T, sep = "\t")
  colnames(acsdems)<-colnames(cols_acsdems)
  acsdems<-acsdems[,-c(which(colnames(acsdems) %in% c('collection_id','dataset_id')))]
  
basedems<-read.delim(paste0(datapath, 'pdem02.txt'), skip=2, header=F, sep = "\t")
cols_basedems<-read.delim(paste0(datapath, 'pdem02.txt'), header=T, sep = "\t")
colnames(basedems)<-colnames(cols_basedems)
basedems<-basedems[,-c(which(colnames(basedems) %in% c('collection_id','dataset_id')))]

longdems<-read.delim(paste0(datapath, 'abcd_lpds01.txt'), skip=2, header=F, sep = "\t")
cols_longdems<-read.delim(paste0(datapath, 'abcd_lpds01.txt'), header=T, sep = "\t")
colnames(longdems)<-colnames(cols_longdems)
longdems<-longdems[,-c(which(colnames(longdems) %in% c('collection_id','dataset_id')))]

lt_site<-read.delim(paste0(datapath, 'abcd_lt01.txt'), skip=2, header=F, sep = "\t")
cols_lt_site<-read.delim(paste0(datapath, 'abcd_lt01.txt'), header=T, sep = "\t")
colnames(lt_site)<-colnames(cols_lt_site)
...
### Household income
household.income = alldems$demo_comb_income_p_all
...
alldems<-merge(alldems, acsdems[acsdems$eventname=='baseline_year_1_arm_1',c('src_subject_id','rel_family_id')])
alldems$eventname<-alldems$eventname.x

famtmp<-data.frame(alldems[,c('src_subject_id','eventname','rel_family_id')])
